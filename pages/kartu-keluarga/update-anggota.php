...
if($db->query("SELECT id_kepala_keluarga FROM kartu_keluarga WHERE id_kepala_keluarga = '$id_warga'")->num_rows > 0) {
  echo "<script>window.alert('Warga sudah menjadi Kepala keluarga!'); window.location.href='../kartu-keluarga/edit-anggota.php?id_keluarga=$id_keluarga'</script>";
  exit;
}

if($db->query("SELECT id_warga FROM warga_has_kartu_keluarga WHERE id_warga = '$id_warga'")->num_rows > 0) {
  echo "<script>window.alert('Warga sudah menjadi anggota keluarga lain!'); window.location.href='../kartu-keluarga/edit-anggota.php?id_keluarga=$id_keluarga'</script>";
  exit;
}
...
if ($hasil == true) {
  echo "<script>window.alert('Berhasil menambahkan anggota!'); window.location.href='../kartu-keluarga/index.php?id_kelurga=$id_keluarga'</script>";
} else {
